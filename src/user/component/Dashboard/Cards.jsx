            ...
            {showproduct?.slice(0,count).map((product) => (
             ...
             <Link to={`/details/${product._id}`}> 
               <div className="shadow-md shadow-gray-500 bg-white p-3 rounded-[10px]  hover:bg-sky-100 hover:text-sky-900 hover:shadow-md text-center">
                {/* like */}
                {user?.wishlist?.includes(product._id)? <FaHeart  className="relative top-7 left-[90%]  text-red-500 " onClick={()=>{wishlist(product._id).then((data)=>{login(data.User)})}}/>:
                <IoIosHeartEmpty  className="relative top-7 left-[90%] text-gray-600" onClick={()=>{wishlist(product._id).then((data)=>{login(data.User)})}}/>}

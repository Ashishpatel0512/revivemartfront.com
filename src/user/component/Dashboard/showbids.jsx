    ...
    <div className='fixed top-[20%] left-[30vw] h-[60%] w-[50vw] bg-white shadow-xl  shadow-black grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2'>
                 ...
                 <div className='flex justify-center gap-10 sticky top-0'>
                      <div className='mt-10 ml-10'>
                         <img src={products?.imageSrc[0]} alt=""  className='h-10 w-10 mb-5  border-gray-300 border-2 rounded-[5px] hover:border-black ' onClick={()=>{setImg(products.imageSrc[0])}}/>
                         <img src={products?.imageSrc[1]} alt="" className='h-10 w-10 mb-5 border-gray-300 border-2 rounded-[5px] hover:border-black' onClick={()=>{setImg(products.imageSrc[1])}}/>
                         <img src={products?.imageSrc[2]} alt="" className='h-10 w-10 mb-5 border-gray-300 border-2  rounded-[5px] hover:border-black' onClick={()=>{setImg(products.imageSrc[2])}}/>
                         <img src={products?.imageSrc[3]} alt="" className='h-10 w-10 mb-5 border-gray-300 border-2  rounded-[5px] hover:border-black' onClick={()=>{setImg(products.imageSrc[3])}}/>
                     </div>
                      <div className='mt-20'>
                      <img src={img} alt="" className='h-[20vh] w-[15vw] border-gray-500 border-2 p-2 shadow-lg shadow-black rounded-[10px] '/>
                     </div>
                </div>
                {/* side2 */}
                <div className='h-[100%] w-full bg-gray-300 text-center overflow-y-auto'>
                   ...
                   <img src={products?.owner?.image} alt="" className='h-[100%] w-[25%]' />
                   <div>
                     <p className='m-2'>{products?.owner?.name}</p>
                     <p className='m-2'>{products?.owner?.phone}</p>
                   </div>
                   <div className='h-[100%] w-[30%] bg-gray-200'>
                     <p className='text-auto font-bold mt-2'>5000$</p>
                     <button className='text-sm   mt-5 text-gray-400 p-1 rounded-[5px]' onClick={()=>{setreadmsg("1")}} >read message</button>
                   </div>
                   {/* readmsg component */}
                   {readmsg==="1"?
                   <div className='absolute top-0 left-0 bg-gray-400 text-white  h-[100%] w-[100%] overflow-y-auto'>
                     <IoChevronBackCircle className=' text-xl sticky  top-2 left-2 ' onClick={()=>{setreadmsg("")}} />
                    <p className='font-mono'>hello , i am ashish patel i am intrested this product please sell  this is interesed i am intrested of the this thst is are..</p>
                   </div>
                   :""}
                   {/*  */}
                  </div>

  ...
  return (
    ...
    <div className='h-[22vh] w-[85vw] bg-gray-50  border-gray-200'>
      <div className='h-auto w-auto pt-5 ml-10 flex justify-left items-center gap-2'>
        <img src={user?.image?.url} alt="" className='h-[15vh] w-[10vw] rounded-[10px] shadow-lg shadow-gray-300' />
    ...
    <div className=' h-[78vh] w-[85vw] border-gray-300 border-t-2 overflow-y-auto'>
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-10 mt-10 ml-10 mr-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-20">
                            {showproduct.slice(0,count).map((product) => (
                                ...
                                <div><MdDelete className='text-xl text-gray-500' onClick={()=>{deleteproduct(product._id).then((data)=>{setdelproduct(!delproduct)})}}/></div>
                                <MdEdit className='text-xl text-gray-500' />
                                </div>
                               {productid===product._id?<Showbids productid={productid} product={product}/>:""}
                              </div>
       ...
       <div className="grid grid-cols-1 gap-x-6 gap-y-10 mt-10 ml-10 mr-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-20">
                            {showbids.slice(0,count).map((bid) => (
                                ...
                                <MdLocalOffer className='text-xl'/>
                                 <h1 className='font-mono bg-sky-700 text-white mb-2 text-lg font-bold rounded-[5px] p-2'><p className='text-sm text-gray-300'>OFFER-PRICE</p>{bid.bidamount}&#8377;</h1>
                                </div>
                                ...
                                <img
                                  ...
                                  className="aspect-square w-full rounded-lg bg-gray-200 object-cover group-hover:opacity-75 xl:aspect-7/8"
                                />
                                <h3 className="mt-4 text-sm text-gray-700">{bid.Productid.name}</h3>
                                <p className="mt-1 text-lg font-medium text-gray-900">{bid.Productid.price}</p>
    ...
    <Dropdown form={form} setform={setform} uploadform={uploadform} setuploadform={setuploadform} updateform={updateform} setupdateform={setupdateform}/>
